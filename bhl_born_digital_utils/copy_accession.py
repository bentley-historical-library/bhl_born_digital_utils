...
def get_log_filepath(accession_number):
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    log_dir = os.path.join(base_dir, "logs")
    log_name = accession_number + "_" + datetime.now().strftime("%Y%m%d%H%M%S")
    log_path = os.path.join(log_dir, "{}.txt".format(log_name))
    return log_path
...
def get_robocopy_command(src, dst):
    ...
    log_path = get_log_filepath(accession_number)
